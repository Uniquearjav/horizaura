    ...
    return (
        <div key={product["asin1"]} className="products w-72 bg-white dark:bg-black shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
            <Link href={`/store/${product["asin1"]}`}>
              <Image src={`${product["image_url"]}`} alt={`${product["item-name"]}}, ID - ${product["product-id-type"]}, Price - Rs. ${product["price"]}`} width={288} height={320} className="h-80 w-72 object-cover rounded-t-xl" />
